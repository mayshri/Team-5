...
docker run --rm -d -p 3000:3000 -v grafana:/var/lib/grafana --env GF_SECURITY_ADMIN_PASSWORD="team2" --env GF_SECURITY_ADMIN_USER="team2" --name grafana grafana/grafana
sleep 1
...
apikeyjson=$(curl -s -X POST -H "Content-Type: application/json" -d '{"name":"apikeycurl", "role": "Admin"}' http://team2:team2@localhost:3000/api/auth/keys)
sleep 1
